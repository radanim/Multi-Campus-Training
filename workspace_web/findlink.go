...
import ( // 패키지 함수 structure 가져오기 
	"encoding/json" // 50 .json 데이터파일 호출하는 unmarshal을 포함하는 패키지 
	"flag" // 29~30 string 함수 호출 패키지, 옵션값 받아오는 패키지 
	"fmt"
	"io/ioutil"
	"math"
	"strconv"
...
func main() {
	...
	_ = json.Unmarshal([]byte(file), &data) // Json 데이터 로드 완료, 데이터 로드하는 함수 unmarshal
